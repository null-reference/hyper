...
export function showPreferences() {
  ...
  // the windows command relies upon the existence of the "editor" environment variable
  // that points to the desired editor to use (ex. C:\Program Files\Sublime Text 3\sublime_text.exe)
  // eslint-disable-next-line no-template-curly-in-string
  const command = isWin ? ' start "%editor%" "%userprofile%\\.hyper.js"' : ' bash -c \'exec env ${EDITOR:=nano} ~/.hyper.js\'';
  const message = [];
  message.push(isWin ?
    ' echo Attempting to open ^%userprofile^%\\.hyper.js with ^%editor%^' :
